...
4. **Bug Fix - WY_RPT TX Power Too High For New Module:** The 30dBm build's `LORA_TX_POWER` was still set to 20, calibrated for the old E22-900M30S's PA. The E22P-433M30S's own datasheet only characterizes up to +12dBm chip-level input, and driving it 8dB past that is a likely cause of the weak/one-sided TX signal seen in testing - possibly PA damage on the tested unit from the earlier overdrive. Lowered to 12 to match the module's documented range; applies to the repeater and both companion (USB/BLE) builds, since all three share the same base config. Still under investigation.
