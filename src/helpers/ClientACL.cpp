  ...
  if (_fs->exists("/s_contacts")) {
    ...
    if (file) {
      ...
      while (!full) {
        ...
        // Zero out shared secret calculation - OTA remote administration is disabled
        memset(c.shared_secret, 0, PUB_KEY_SIZE);
...
bool ClientACL::applyPermissions(const mesh::LocalIdentity& self_id, const uint8_t* pubkey, int key_len, uint8_t perms) {
  ...
  if ((perms & PERM_ACL_ROLE_MASK) == PERM_ACL_GUEST) { // guest role is not persisted in contacts
    ...
    memset(c->shared_secret, 0, PUB_KEY_SIZE); // Zero out shared secret
  }
  return true;
}
